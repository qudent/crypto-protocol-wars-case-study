...
We propose that the partial success of the technologists in this case — in contrast to the total failure of the nuclear scientists — can be attributed to several structural differences: the technologists could *build and deploy* their preferred outcomes rather than merely advise decision-makers; the technology created value for a dispersed commercial constituency with independent political power (unlike nuclear weapons, whose value accrued to the state itself); and — most fundamentally — the technologists had already won the prior battle over network architecture, which gave them a permissionless platform on which to distribute their tools without institutional approval. The protocol wars were the precondition for the crypto wars; the crypto wars were fought on terrain that the internet engineers had already shaped. The analogy to contemporary AI governance is direct and urgent.
...
Simultaneously, the internet's predecessor networks were being designed. The ARPANET, funded by the Department of Defense's Advanced Research Projects Agency (DARPA), used the TCP/IP protocol suite developed primarily by Vint Cerf and Bob Kahn. The architectural choices made in these protocols — particularly the end-to-end principle, which holds that intelligence should reside at the endpoints of a network rather than in the network itself — would have profound political consequences. A "dumb" network that simply moves packets without inspecting them is a network that cannot easily censor, surveil, or restrict what its users do. As Lawrence Lessig would later argue in "Code is Law" (2000), the architecture of a network functions as a form of regulation as powerful as any statute. The internet's designers embedded permissionlessness into the infrastructure itself — though the full implications of this choice would not become apparent until the crypto wars began two decades later.
...
Running in parallel with the crypto wars was a less publicized but equally consequential battle over the architecture of global computer networking — one whose outcome, it can be argued, *determined* whether the crypto wars could even be fought.

Governments and established telecommunications companies, through the International Organization for Standardization (ISO), had developed the Open Systems Interconnection (OSI) model — a seven-layer protocol stack designed through a formal, top-down standards process involving national delegations and official representatives. The OSI model was not merely a different technical standard — it reflected a fundamentally different political vision of networking, one that mirrored the existing telecommunications order. In the telco model, networks were operated by state monopolies or state-regulated carriers (the European Post, Telegraph and Telephone agencies, or PTTs). Users were identified and billed per-connection. The network operator controlled what services could run on the network. International connectivity was negotiated between national carriers through bilateral agreements.

Had OSI prevailed, the political architecture of global networking would have been radically different from what we know today. There would likely be no pseudonymity against the state, since every network connection would be tied to a billed, identified subscriber. International access might be metered and restricted — an "EU+" package to access five websites outside your home country per day is not a parody but a straightforward extrapolation of the telco billing model to data networking. There would be no VPNs that Chinese dissidents could use to circumvent state censorship, because the network itself would track users at every layer in order to bill them. Setting up a server would likely require renting space in a telco-owned datacenter and completing months of compliance processes, because the network operator, not the end user, would determine what services were permissible. There would be no Wikipedia, no internet startups founded in dorm rooms, no permissionless innovation — because the network would not be permissionless.

This is not purely speculative. Two real-world systems illustrate what an OSI-style internet would have looked like. France's **Minitel** (1978-2012) was a centralized, state-operated data network run by France Télécom: all services were hosted by the carrier, users were identified and billed per-minute, and launching a new service required approval from the operator. Minitel was commercially successful within France but was a walled garden that produced no equivalents of the World Wide Web, open-source software, or peer-to-peer communication. China's **Great Firewall** is often cited as proof that a state can control the internet — but it actually demonstrates the opposite. China built its censorship apparatus *on top of* TCP/IP, and precisely because TCP/IP is the underlying architecture, the firewall is porous: hundreds of millions of Chinese users routinely circumvent it using VPNs, and the government largely tolerates this because TCP/IP-based infrastructure is too economically valuable to replace with something more controllable. The Great Firewall can make access to the open internet inconvenient, but it cannot make it impossible — because TCP/IP was not designed to give intermediaries the power to prevent endpoint-to-endpoint communication. Had China built its network from scratch on an OSI/telco model, there would be nothing to circumvent: the network itself would enforce identity and access control at every layer, and a "VPN" would be a meaningless concept. The open internet is not a law of nature; it is the product of specific architectural choices that were contested and could have gone the other way — but having won, those choices constrain even the most powerful states.

This point has a crucial implication for the crypto wars: **the crypto wars were winnable by the technologists in large part because the protocol wars had already been won.** Phil Zimmermann could distribute PGP by uploading it to the internet because TCP/IP's architecture allowed anyone to publish anything without asking permission from a network operator. Had the internet run on OSI-style protocols controlled by national PTTs, Zimmermann would have needed institutional approval to distribute his software — and would never have received it. The permissionless architecture of TCP/IP was the substrate on which the entire crypto wars resistance was built.

The U.S. government mandated the use of OSI protocols through the Government OSI Profile (GOSIP), requiring all government computer networks to adopt OSI standards. European governments, working through their PTT monopolies, pushed even harder for OSI adoption. The expectation was that OSI would become the global standard, and TCP/IP — developed by a small community of researchers through the informal Internet Engineering Task Force (IETF) — would be relegated to a historical footnote.
...
The cryptographers, by contrast, could write code and publish algorithms. Phil Zimmermann did not need to persuade the President to allow strong encryption — he simply wrote PGP and released it. Diffie and Hellman did not need government permission to publish their paper on public-key cryptography — they published it in an academic journal. The internet engineers did not need a government mandate to deploy TCP/IP — though they did need government *research funding* (DARPA, NSF) to build the initial infrastructure and reference implementation.
...
Crucially, this capacity depended on a prior victory: the protocol wars. Zimmermann could upload PGP to the internet and have it reach millions of users only because the internet ran on TCP/IP — a permissionless architecture in which any user could publish anything without approval from a network operator. Had the internet been built on the telco-controlled OSI model, distributing unauthorized encryption software would have required institutional cooperation that would never have been granted. The "build and deploy" strategy was not available to technologists in general; it was available specifically because the internet's architecture had already been decided in their favor.

It is important to note, however, that the crypto wars and the protocol wars involved different scales of effort. Phil Zimmermann could write PGP alone on his laptop. No individual could build a TCP/IP network. The protocol wars required DARPA research funding, university teams (the Berkeley BSD implementation was the decisive artifact), NSF investment in backbone infrastructure, and eventually commercial ISP buildout. But the decisive battles were fought at the *specification and software implementation* layer — where a small research community could operate — not at the physical infrastructure layer, which required major investment but was protocol-agnostic: the same cables could carry TCP/IP or OSI packets. The Berkeley team's decision to ship a free TCP/IP stack with BSD Unix settled the protocol question at a scale where a few dozen engineers were decisive. Once that battle was won, the crypto wars could be fought by individuals.
...
First, *mathematical* resistance: cryptographic algorithms are mathematical ideas. Once published, they cannot be unpublished. Software can be copied at zero marginal cost. This is an inherent property of information — you cannot classify mathematics.

Second, *architectural* resistance: the internet was *designed* to be permissionless. This was not an inherent property of networking technology; it was the consequence of specific design choices made by TCP/IP's creators — choices that were contested and nearly overturned in the protocol wars. As David Isenberg argued in his 1997 paper "Rise of the Stupid Network," the internet's value came precisely from being "stupid" — a dumb pipe that moves packets without inspecting or controlling them. A "smart" telco-style network would have offered the state control points at which to enforce restrictions on what users could publish or access. As John Gilmore famously observed, the internet treats censorship as damage and routes around it — but this is a property of TCP/IP's architecture, not of networks in general. States can layer censorship on top of TCP/IP (as China's Great Firewall does), but because the underlying protocol was not designed to give intermediaries control over endpoint communication, such censorship is inherently leaky — circumventable by VPNs and other tools that exploit TCP/IP's fundamental indifference to what its packets contain. A network built from the ground up on telco principles would not have this vulnerability.
...
But Zimmermann's success also depended on something Szilard lacked entirely: a permissionless distribution platform. Zimmermann uploaded PGP to a BBS and later to the internet; within days it had spread worldwide, and the government's only recourse was the slow machinery of criminal investigation. Had Zimmermann needed to distribute PGP through institutional channels — a software publisher, a telecommunications carrier, a government-approved distribution system — the investigation would have shut him down before PGP reached a meaningful audience. The internet's architecture was as essential to Zimmermann's success as the mathematics of RSA.
...
**The protocol wars as precondition for the crypto wars**: It is worth emphasizing that the outcome of the protocol wars shaped the battlefield for every subsequent conflict described in this case study. Had OSI won, Phil Zimmermann could not have uploaded PGP to the internet — because there would have been no permissionless internet to upload it to. Daniel Bernstein could not have argued that publishing encryption source code is protected speech — because "publishing" on an OSI network would have required institutional authorization. The Clipper Chip might have succeeded, because in a telco-controlled network, the government would not have needed to persuade consumers to adopt it voluntarily — the carrier would simply have mandated it. The entire structure of the crypto wars — individual technologists against the state, with the open internet as the arena — was a consequence of TCP/IP's victory. In this sense, the protocol wars were the most consequential of all the battles described here, because they determined the terms on which everything else would be fought.
...
The net neutrality debate is best understood as a *rearguard action in the protocol wars*. The ISPs — the spiritual descendants of the PTT monopolies that backed OSI — were essentially attempting to re-impose the telco model at the infrastructure level: a "smart" network that inspects, prioritizes, and charges differently for different types of traffic, rather than the "stupid" TCP/IP pipe that treats all packets equally. Net neutrality advocates were fighting to preserve the permissionless character of the internet that TCP/IP had established — the very architecture that made the crypto wars winnable.
...
| **Distribution platform** | Government-controlled (classified labs, military bases) | Permissionless internet (consequence of protocol wars victory) |
...
3. **They had no permissionless distribution platform**. The Manhattan Project scientists worked within a classified, government-controlled infrastructure. There was no way for Szilard to bypass the chain of command and distribute his ideas directly to the public or to allied scientists in other countries. The cryptographers operated on the internet — a permissionless platform where anyone could publish anything. This was not an accident but a consequence of the protocol wars: TCP/IP's architecture, by design, did not require institutional permission to publish. Building this platform was itself a major institutional effort — DARPA funding, university teams, NSF backbone investment — but it was a different *kind* of institutional effort: research-driven rather than military-driven, and it produced a platform whose permissionless character then enabled individual action by the cryptographers.
...
**Like the telco model, not the internet model**: Here the analysis of the protocol wars becomes directly relevant. Frontier AI models are not distributed on a permissionless platform. They are accessed through cloud APIs, enterprise contracts, and app stores — all of which have gatekeepers. This is structurally closer to the telco-controlled network that OSI would have created than to the permissionless internet on which PGP was distributed. The government can pressure these gatekeepers (as the supply chain risk designation does) in a way that it could never pressure the internet's architecture. Open-source AI models (Meta's Llama, Mistral, etc.) are the exception — they represent the crypto-wars-like vector, distributable without institutional permission. The critical question for AI governance may therefore be whether open-source models become "good enough" to matter. If they do, AI may follow the crypto trajectory: too widely distributed to control. If frontier models maintain a decisive advantage, the government retains the chokepoints it needs to enforce its will — and the AI story will look more like the nuclear story than the crypto story.
...
**4. Don't rely on the goodwill of any single company**. Both Schneier and the EFF made this point independently in their analyses of the Anthropic standoff. Schneier argued that democratic legal structures, not corporate ethics, should govern these questions: "we should strengthen the legal protections around government procurement." The EFF warned that "the state of your privacy is being decided by contract negotiations between giant tech companies" and that imposing restrictions is "properly a role for Congress and the courts, not the private sector." OpenAI's willingness to fill the gap left by Anthropic's principled stand illustrates the fragility of corporate ethics as a substitute for legal protections — the same lesson the crypto wars taught when the EFF fought to embed privacy protections in law rather than leave them dependent on individual companies' goodwill.
